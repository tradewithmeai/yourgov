...
def _ensure_seed_db():
    ...
    Decompress yourgov.db.gz -> yourgov.db on first use. Prefer the raw file when it
    ...
    candidates = ["/tmp/yourgov.db", raw] if os.path.exists("/tmp") else [raw]
...
_WRITABLE_DB = "/tmp/yourgov.db" if os.path.exists("/tmp") else _SEED_DB
